...
OUTPUT_DIR = Path("results/milton_full_sequence")
OUTPUT_CSV = OUTPUT_DIR / "milton_full_sequence_results.csv"
...
def main():
    ...
    milton_images = sorted(
        IMAGE_DIR.glob("*milton_full_*.png"),
        key=lambda p: extract_time(p.name)
    )

    print("=" * 72)
    print("MILTON FULL-DISK SEQUENCE TEST")
    print("=" * 72)

    print(f"Device: {DEVICE}")
    print(f"Images found: {len(milton_images)}")
    print(f"Confidence threshold: {CONF_THRESHOLD}")
    print()

    if not milton_images:
        print("ERROR: No Milton images found.")
        return
    ...
    for number, image_path in enumerate(milton_images, start=1):

        timestamp = extract_time(image_path.name)

        print(
            f"[{number:02d}/{len(milton_images)}] "
            f"{timestamp}",
            end=" "
        )

        full_image, raw, merged = detect_full_disk(
            model,
            image_path
        )
        ...
        if merged:
            ...
            for eye_number, detection in enumerate(
                ...
                # Larger display marker so it is visible
                marker_radius = 18
    ...
    print(f"Total images:             {len(milton_images)}")
    print(f"Images with detection:    {total_with_detection}")
    print(f"Images without detection: {total_without_detection}")
    print(f"Images with >1 candidate: {total_multiple}")
